...
class SnapshotCopyEndToEndTest(unittest.TestCase):
  """End to end test on AWS.
  ...
  def setUpClass(cls):
    try:
      project_info = utils.ReadProjectInfo(['s3_destination', 'zone',
        'snapshot_id'])
    except (OSError, RuntimeError, ValueError) as exception:
      raise unittest.SkipTest(str(exception))
    cls.zone = project_info['zone']
    cls.s3_destination = project_info['s3_destination']
    cls.snapshot_id = project_info['snapshot_id']
    cls.subnet_id = project_info.get('subnet_id', None)
    cls.security_group_id = project_info.get('security_group_id', None)
